...
| `backend` | PennyLane QNode interface. Default and maintained value is `autograd`. `jax` is an opt-in alternative: analytic execution only (`shots` must be `<=0`, rejected otherwise), runs under a separate `pennylane-gpu-jax-sep2026` conda env (pinned `jax==0.10.2` -- do not let it float to a newer version), trains with `optax.adam` (wrapped in `optax.inject_hyperparams` for learning-rate decay) instead of `qml.AdamOptimizer`, and has its own checkpoint schema (`optimizer.name == 'optax_adam'`). The training step is `jax.jit`-compiled. See `llm_summary.MD` for the full design and known pitfalls. |
